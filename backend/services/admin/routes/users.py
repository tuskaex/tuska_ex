...
from dependencies import require_permission, require_user_in_scope
...
async def list_users(
    ...
    # tenant_safe: list_users applies scope_filter() for a sub_admin.
    admin: User = Depends(require_permission("users.view", tenant_safe=True)),
...
async def get_user_detail(
    user_id: uuid.UUID,
    admin: User = Depends(require_user_in_scope("users.view")),
    db: AsyncSession = Depends(get_db),
):
...
async def add_fund(
    ...
    body: FundRequest,
    request: Request,
    admin: User = Depends(require_user_in_scope("users.add_fund")),
    db: AsyncSession = Depends(get_db),
):
...
async def deduct_fund(
    ...
    body: FundRequest,
    request: Request,
    admin: User = Depends(require_user_in_scope("users.deduct_fund")),
    db: AsyncSession = Depends(get_db),
):
...
async def ban_user(
    user_id: uuid.UUID,
    request: Request,
    admin: User = Depends(require_user_in_scope("users.ban")),
    db: AsyncSession = Depends(get_db),
):
...
async def block_trading(
    user_id: uuid.UUID,
    request: Request,
    admin: User = Depends(require_user_in_scope("users.block_trading")),
    db: AsyncSession = Depends(get_db),
):
...
async def kill_switch(
    user_id: uuid.UUID,
    request: Request,
    admin: User = Depends(require_user_in_scope("users.kill_switch")),
    db: AsyncSession = Depends(get_db),
):
...
async def login_as_user(
    user_id: uuid.UUID,
    request: Request,
    admin: User = Depends(require_user_in_scope("users.impersonate")),
    db: AsyncSession = Depends(get_db),
):
...
async def delete_user(
    ...
    admin: User = Depends(require_user_in_scope("users.delete")),
    db: AsyncSession = Depends(get_db),
):
