# Swisscresta APK — Vantage-style UI Redesign

**Date:** 2026-06-03
**Status:** Approved for implementation planning
**Scope:** Mobile APK only (`Swisscresta_apk/`). Web project (`SwissCresta/`) is reference-only for backend API endpoints.

---

## 1. Goal

Rebuild the Swisscresta mobile app's UI to match the visual language of Vantage (CFD trading app) while keeping only the features that exist in the Swisscresta backend. The current PipHigh-fork UI is replaced with a design system extracted from Vantage screenshots: pure-black background, orange (`#F26A1F`) accent, sparkline-rich watchlists, a floating pill bottom-nav, and the signature Sell-pink / Buy-grey split order ticket.
...
### Out of scope
- Web project (`SwissCresta/`) — no changes.
- Backend changes — none. Backend endpoints already match what the app calls.
- Features that exist in Vantage but not in Swisscresta backend: Earn tab (crypto savings APR), FX TV, Webinar, Analysis articles, dedicated news feed.
...
| `SpotlightCard` | Card with title + items + glowing Swisscresta wordmark badge |
...
| WatchlistList | SecureStore `@swisscresta/watchlist` + `GET /instruments/prices/all` + WS `/ws/prices` patch |
| Sparkline per row | `GET /instruments/{symbol}/bars?resolution=60&limit=24` (cached 60 s) |

**Behaviors:** pull-to-refresh re-fetches; live ticks patch rows individually via `React.memo`; hidden-balance state persisted to SecureStore (`@swisscresta/balance-hidden`).
...
| Q4 | Backend "Shares" segment availability — does Swisscresta list equities? | Inspect `GET /instruments` response in Phase 3; hide Shares tab if no rows. |
