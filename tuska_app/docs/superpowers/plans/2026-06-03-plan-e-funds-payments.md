...
**Goal:** Replace the Funds placeholder with the real Vantage-style funds tab, wired to all 3 payment gateways already in the Swisscresta backend (Razorpay, on-chain USDT, manual bank/UPI) plus withdraw / transfer / transaction history. **This is the phase that wires the actual payment integrations the user originally asked for.**
...
```bash
cd /Users/tarundewangan/Downloads/Projects/Swisscresta/Swisscresta_apk && npx expo install react-native-qrcode-svg
```
