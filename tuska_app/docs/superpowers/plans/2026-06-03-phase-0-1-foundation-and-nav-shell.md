...
**Goal:** Ship a runnable Swisscresta APK with the Vantage design system in place (tokens + 25 reusable components) and the new 4-tab floating bottom-nav navigation shell hooked up to placeholder tab screens.
...
**Working directory:** `/Users/tarundewangan/Downloads/Projects/Swisscresta/Swisscresta_apk` for all paths below (paths shown relative to it).
...
export default function SpotlightCard({ title = 'Spotlight', items, brandLabel = 'Swiss' }) {
  return (
    <Card padding={0}>
...
Log in with a valid Swisscresta backend account. Expected:
