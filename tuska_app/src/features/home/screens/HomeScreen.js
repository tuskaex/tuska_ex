...
export default function HomeScreen() {
  ...
  // `unrealized_pnl` is the field /accounts/{id}/summary actually returns
  // (see AccountSummary in packages/common/src/schemas/trading.py).
  //
  // This used to look for `floating_pnl` and then `open_pnl`. Neither exists
  // in that response, so it ALWAYS fell through to `equity - balance` — and
  // the backend defines equity as `balance + credit + unrealized_pnl`, so the
  // difference is `credit + unrealized_pnl`. Any account carrying credit (a
  // bonus, and most demo accounts) had its credit reported as floating P/L,
  // which is why the card never matched the positions list.
  //
  // The subtraction is kept as a fallback for older backends, with credit
  // removed so it means the same thing as the field above it.
  const acctCredit = num(accountSummary?.credit) ?? 0;
  const acctFloatingPnl =
    num(accountSummary?.unrealized_pnl)
    ?? ((acctEquity != null && acctBalance != null)
      ? (acctEquity - acctBalance - acctCredit)
      : null);
    ...
    <Screen edges={['top']} glow={false}>
      ...
      >
        ...
        >
          ...
          {/* Brand mark — top-right, mirroring the chip in the opposite
              corner. The white cut-out rather than the colour version: the
              card is brand blue in both themes, so a blue-on-blue mark would
              disappear and the mark must not follow the theme. */}
          <Image source={brandAssets.markWide} style={styles.cardLogo} resizeMode="contain" />
...
const styles = StyleSheet.create({
  ...
  cardLogo: {
    ...
    top: space.xl,
    right: space.xxl,
    // 910:365 is the trimmed mark's own ratio, so `contain` has no empty
    // band and the box is exactly the mark.
    width: 80,
    height: 32,
  },
  chip: {
