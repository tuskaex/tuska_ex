...
import DemoFundingGate from '../components/DemoFundingGate';
...
export default function DepositScreen() {
  ...
  // The account group's minimum, enforced the same way the web wallet does
  // (`depositMinDeposit` there). Without it the app happily submitted a $5
  // deposit against a group with a $100 floor and let the backend or an
  // admin reject it later.
  const minDeposit = useMemo(
    () => Number(selectedAccount?.account_group?.minimum_deposit ?? 0) || 0,
    [selectedAccount],
  );
  const belowMin = minDeposit > 0 && Number(amount) > 0 && Number(amount) < minDeposit;
  ...
  const openGateway = useCallback(() => {
    ...
    if (belowMin) {
      showToast({ kind: 'warn', message: `Minimum deposit for this account is $${minDeposit}` });
      return;
    }
    nav.navigate('DepositOxapay', { amount: Number(amount) });
  }, [amount, belowMin, minDeposit, nav]);
  ...
  const submitManual = useCallback(async () => {
    ...
    if (belowMin) { showToast({ kind: 'warn', message: `Minimum deposit for this account is $${minDeposit}` }); return; }
  ...
  }, [amount, belowMin, minDeposit, hasDestination, txId, proof]);
  ...
  // Live accounts only, matching the web wallet.
  if (user?.is_demo) {
    return (
      <Screen edges={['top']}>
        {header}
        <DemoFundingGate feature="Deposits" />
      </Screen>
    );
  }
  ...
  return (
    <Screen edges={['top']}>
      ...
      <ScrollView contentContainerStyle={{ padding: space.lg, paddingBottom: BOTTOM_NAV_PILL_HEIGHT + space.huge }} keyboardShouldPersistTaps="handled">
        ...
        />
        {minDeposit > 0 ? (
          <Text style={[styles.minNote, belowMin && styles.minNoteBad]}>
            {belowMin
              ? `Minimum deposit for this account is $${minDeposit}`
...
const styles = StyleSheet.create({
  ...
  minNote: { color: vantage.textMuted, fontFamily, fontSize: sizes.label, marginTop: space.xs },
  minNoteBad: { color: vantage.down },
