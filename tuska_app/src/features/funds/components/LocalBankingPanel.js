...
import { fetchKycStatus, isKycApproved } from '../../../utils/kycGate';
...
export default function LocalBankingPanel({ amount = '' }) {
  ...
  // Local Banking is KYC-gated on the server. The web checks before showing
  // the form (wallet/page.tsx: `if (!kycApproved) … router.push('/kyc')`);
  // this panel only found out from the 403 after the user had filled it in.
  // `unknown` (the request failed) is treated as not-approved by
  // isKycApproved, so a flaky connection never opens the gate by accident.
  const [kycOk, setKycOk] = useState(true);
  useFocusEffect(useCallback(() => {
    let alive = true;
    loadRequests();
    fetchKycStatus()
      .then((st) => { if (alive) setKycOk(isKycApproved(st)); })
      .catch(() => {});
    return () => { alive = false; };
  }, [loadRequests]));
  ...
  const sendRequest = useCallback(async () => {
    if (!kycOk) {
      showToast({ kind: 'warn', message: 'Complete KYC verification to use Local Banking deposits' });
