...
import { netPnl, openPnl } from '../../../utils/pnl';
...
function buildHistoryHtml(trades, rangeLabel, account) {
  // Closed trades, so NET — via the shared helper rather than repeating the
  // arithmetic, which is how the statement and the on-screen history drift.
  const netOf = (t) => netPnl(t) ?? 0;
...
<style>
  ...
  <div class="brand">
    ...
    <img src="${TRADE_WEB_URL}/marketing/speedtrade-logo.png" alt="" onerror="this.style.display='none'"/>
  </div>
...
// Floating P&L for an OPEN position. Delegates to the shared helper so this
// file and the web terminal cannot drift.
//
// This used to compute `profit - commission + swap` — the CLOSED-trade
// formula — for open positions. Commission is already taken off the balance
// when the position opens and swap when the overnight engine runs, so that
// charged both a second time: the trade blotter's total disagreed with the
// account's own equity by exactly the commission on the open trades.
function plOf(p) {
  return openPnl(p);
}
...
function PositionRow({ position, onClose, onSetSlTp }) {
  ...
  // GROSS P&L for this OPEN position — the same figure the row behind this
  // sheet shows, and the same one the web terminal and the desktop terminal
  // show (`openPnl` in utils/pnl.js explains why open and closed differ).
  //
  // This used to subtract commission and add swap, i.e. the CLOSED-trade
  // formula. Both charges are already off the balance by now, so taking them
  // out again double-charged them: the sheet quoted a worse P&L than the row
  // it was opened from, and than the estimate shown for a partial close.
  // The fees still appear on their own rows below, which is where they
  // belong.
  const pl = openPnl(position);
...
function HistoryRow({ trade }) {
  ...
  // NET realized P&L for a closed trade — costs included, via the shared
  // helper (see utils/pnl.js for why open and closed differ).
