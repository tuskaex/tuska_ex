    ...
    <div className="relative w-full overflow-hidden rounded-xl border border-border-primary bg-bg-secondary p-3">
      <div className="flex flex-wrap items-center gap-3 mb-2 text-[10px] text-text-tertiary">
        <span className="flex items-center gap-1">
          <span className="w-2 h-2 rounded-full bg-[#D60101]" /> Strong profit
        </span>
        <span className="flex items-center gap-1">
      ...
      >
        ...
        <defs>
          <linearGradient id="eqFill" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor="#D60101" stopOpacity="0.25" />
        ...
        {pathD ? (
          <path
            ...
            stroke={NEON}
            strokeWidth={2}
            className="drop-shadow-[0_0_8px_rgba(214,1,1,0.35)]"
          />
        ) : null}
        {hover ? (
          <>
        ...
        >
          <div className="p-3 md:p-4 border-b border-border-primary flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-2">
              <Calendar className="w-5 h-5 text-[#D60101]" />
              ...
              {(
                ...
                <button
                  ...
                  className={clsx(
                    ...
                    calView === t.id ? 'bg-[#D60101] text-text-inverse' : 'bg-bg-secondary text-text-tertiary hover:text-text-primary',
                  )}
                >
                  {t.label}
                </button>
          ...
          <div className="px-3 py-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-[11px] border-b border-border-primary bg-bg-secondary/40">
            <span className="text-[#D60101] font-semibold">Monthly P&L {fmtCompactSigned(s.monthlyPnlUsd)}</span>
            ...
            <span className="ml-auto text-text-tertiary">
              <span className="text-[#D60101]">{s.wins}W</span> <span className="text-red-400">{s.losses}L</span>
            </span>
          </div>
                  ...
                  return (
                    <div
                      ...
                      className={clsx(
                        ...
                        inMonth && isWin && 'border-[#D60101]/50 bg-[#D60101]/10',
                          ...
                          {showUsd ? (
                            <span
                              className={clsx(
                                ...
                                cell.pnlUsd! >= 0 ? 'text-[#D60101]' : 'text-red-400',
                              )}
                            >
        ...
        <div className="space-y-3">
          ...
          <div className="rounded-xl p-4 border" style={{ backgroundColor: CARD, borderColor: BORDER }}>
            <div className="flex items-center gap-2 text-sm font-semibold mb-3">
              <Target className="w-4 h-4 text-[#D60101]" />
              Trade win %
            </div>
            <p className={clsx('text-3xl font-bold', d.stats.tradeWinPct >= 50 ? 'text-[#D60101]' : 'text-red-400')}>
              {d.stats.tradeWinPct.toFixed(1)}%
            </p>
            <div className="h-2 rounded-full bg-red-500/40 mt-3 overflow-hidden flex">
              <div
                className="h-full bg-[#D60101]"
                style={{ width: `${Math.min(100, d.stats.tradeWinPct)}%` }}
              />
            </div>
            <div className="flex justify-between text-[11px] mt-2">
              <span className="text-[#D60101]">{j.wins} won</span>
              <span className="text-red-400">{j.losses} lost</span>
            </div>
          </div>
          <div className="rounded-xl p-4 border" style={{ backgroundColor: CARD, borderColor: BORDER }}>
            <div className="flex items-center gap-2 text-sm font-semibold mb-3">
              <BarChart3 className="w-4 h-4 text-[#D60101]" />
              Performance
            </div>
            <ul className="space-y-2 text-sm">
              {[
                ['Profit factor', d.stats.profitFactor.toFixed(2), 'text-[#D60101]'],
                ['Avg win', fmtUsd(d.stats.avgWinUsd), 'text-[#D60101]'],
                ['Avg loss', fmtUsd(-d.stats.avgLossUsd), 'text-red-400'],
                ['Period P&L', fmtCompactSigned(d.stats.periodPnlUsd), 'text-[#D60101]'],
      ...
      <section>
        <div className="rounded-xl p-4 border" style={{ backgroundColor: CARD, borderColor: BORDER }}>
          <div className="flex items-center gap-2 mb-3">
            <LineChart className="w-5 h-5 text-[#D60101]" />
