    ...
    >
      <nav className="w-full mx-auto px-3 md:px-4 lg:px-5 relative flex items-center gap-3 h-16 md:h-[68px]">
        ...
        <div className="hidden lg:flex items-center gap-1.5 xl:gap-2 ml-auto shrink-0">
          ...
          {/* RESTORED by request.
              Caveat that has not gone away: /downloads/TuskaExTerminal-Setup-1.0.5.exe
            ...
            {terminalMenuOpen && (
              <>
                ...
                >
                  ...
                  {/* Windows */}
                  <a
                    href="/downloads/TuskaExTerminal-Setup-1.0.5.exe"
                  ...
                  {/* macOS. Live again: the dmg is signed with our Developer ID
                      AND carries Apple's notarisation ticket, so Gatekeeper lets
                      it through — verified by downloading it, applying the
                      quarantine flag a browser sets, and watching spctl report
                      "accepted / source=Notarized Developer ID" where it used to
                      say "rejected".

                      "universal" in the filename is load-bearing: the image
                      holds an arm64 + x86_64 binary, and package-macos.sh
                      derives that word from `lipo -archs` on the built binary
                      rather than from whichever Mac did the build. A single-arch
                      build therefore ships as -arm64/-x86_64 and this link 404s,
        ...
        <div className={`lg:hidden border-t rounded-b-2xl backdrop-blur-2xl backdrop-saturate-150 ${c.divider} ${c.drawer}`}>
          <ul className="w-full mx-auto px-6 md:px-10 lg:px-16 py-4 flex flex-col gap-3">
            ...
            {/* Terminal download, mobile counterpart of the desktop picker.
                ...
                Both platforms, matching the desktop picker exactly. The two are
                deliberately changed in the same commit: while the picker held
                macOS at "Soon" this pill was removed too, because a mobile menu
                still handing out the dmg would have served the very build the
                picker was withholding — the same site offering different
