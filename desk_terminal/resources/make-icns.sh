#!/usr/bin/env bash
# Builds resources/tuskaex.icns — the macOS app icon — from a square PNG master.
#
...
#
# Source preference:
#   1. resources/tuskaex-1024.png   (ideal — every slot is a true downscale)
#   2. resources/tuskaex-256.png    (what the repo ships; 512/1024 are upscaled)
#
# The 256px master is the app-icon artwork proper: the dark rounded-square tile,
# NOT frontend/trader/public/marketing/tuskaex_fevicon.png, which is 512px but a
...
# Drop a 1024px tile in as tuskaex-1024.png and this script picks it up with no
# other change.
#
...
out="$here/tuskaex.icns"

src=""
for cand in "$here/tuskaex-1024.png" "$here/tuskaex-256.png"; do
...
if (( master_px < 1024 )); then
    ...
    echo "          resources/tuskaex-1024.png for a sharper Dock icon."
fi

work="$(mktemp -d)"
trap 'rm -rf "$work"' EXIT
set="$work/tuskaex.iconset"
mkdir -p "$set"
